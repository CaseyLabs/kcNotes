/*
  ...
  - This is HTMX v2.0.4, copied into the repository for self-hosting.
  - The file is minified (compressed onto one line) for production efficiency.
  - We generally do NOT hand-edit vendored third-party code because updates
    should come from upstream releases to preserve correctness and patchability.

  How to read minified JavaScript as a beginner:
  1. Identify wrapper shape:
     - This file starts with an IIFE-style expression:
         `var htmx = function(){ ... return Q }();`
     - JavaScript concept: functions are values; calling one immediately creates
       private scope and returns a public API object (`htmx`).
  2. Look for public API object:
     - Near the top it builds an object with methods like `on`, `ajax`,
       `find`, `swap`, etc. That object becomes `window.htmx`.
  3. Recognize browser APIs in use:
     - `document.querySelector`, `addEventListener`, `XMLHttpRequest`,
       `FormData`, `history.pushState`, DOM mutation methods.
  4. Understand project interaction:
     - Our project relies on HTMX events and attributes (`hx-*`) for partial
       page updates, while Go handlers return HTML fragments/full pages.
     - Our own script in `web/static/js/app.js` listens to HTMX request config
       events and adds CSRF headers.

  If you want to learn deeply:
  - Read the non-minified HTMX source from upstream docs/repo.
  - Compare one feature at a time (for example, request lifecycle events) with
    usage in this project's templates and handlers.
*/
