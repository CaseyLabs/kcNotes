...
func New(public *handlers.Public, admin *handlers.Admin, staticDir string, mw MiddlewareSet) *Router {
	...
	mux.Handle("GET /admin/mfa", mw.RequireAuth(mw.CSRF(http.HandlerFunc(admin.MFAPage))))
	mux.Handle("POST /admin/mfa/setup", mw.RequireAuth(mw.CSRF(mw.SensitiveLimit(http.HandlerFunc(admin.MFASetup)))))
	mux.Handle("POST /admin/mfa/enable", mw.RequireAuth(mw.CSRF(mw.SensitiveLimit(http.HandlerFunc(admin.MFAEnable)))))
	mux.Handle("POST /admin/mfa/disable", mw.RequireAuth(mw.CSRF(mw.SensitiveLimit(http.HandlerFunc(admin.MFADisable)))))
	mux.Handle("POST /admin/mfa/recovery/regenerate", mw.RequireAuth(mw.CSRF(mw.SensitiveLimit(http.HandlerFunc(admin.MFARegenerateRecovery)))))
